...
'summary':
  'ZEN 是一个结合 AI 功能的静态站点生成器，专为 Markdown 文档设计。它允许用户将包含 Markdown 文件的 Git
  文件夹转换为静态 HTML 站点，无需复杂配置，所有设置隐藏在 .zen 目录中。核心功能包括智能分类导航（使用 AI
  分析内容生成站点地图）、自动增量翻译（支持多语言版本，用户可用母语写作，AI 处理翻译），以及最小化打扰的写作体验。快速开始指南要求安装
  Node.js、获取 OpenAI API Key 和 Git，通过命令行构建站点，支持指定语言和基础 URL。'
'tags':
  - '静态站点生成器'
  - 'Markdown'
  - 'AI'
