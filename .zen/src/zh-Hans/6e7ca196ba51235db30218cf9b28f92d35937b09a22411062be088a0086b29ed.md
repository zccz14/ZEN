...
'summary':
  'ZEN 是一个结合 AI 的静态站点生成器，专注于简化文档写作和构建过程。它允许用户使用 Markdown 在 git
  管理的文件夹中创建静态 HTML 站点，无需复杂配置，所有设置隐藏在 .zen 目录下。核心功能包括智能分类导航（通过 AI
  分析内容生成站点地图）、自动增量翻译（支持多语言版本，用户可用母语写作，AI 处理翻译），以及最小化打扰的写作体验。工具依赖 Node.js、OpenAI
  API Key 和 Git，通过命令行快速构建，支持指定语言参数如 zh-Hans 和 en-US。'
'tags':
  - '静态站点生成器'
  - 'Markdown'
  - 'AI'
